...
const tokens = data.split(/\w/).map((character) => {
  return tokenizer(character);
});

const entryPoint = document.getElementById("entry");

const colors = ["#a9def9", "#ede7b1", "#f694c1", "#e4c1f9", "#d3f8e2"];

tokens.forEach((token) => {
  const block = document.createElement("div");
  if (token.toString() !== "newline") {
    block.className = `block ${token.toString()}`;
    const color = colors[Math.floor(Math.random() * colors.length)];
    block.style.background = color;
  } else block.className = "newline";
  entryPoint.appendChild(block);
});
